"""
Task: libero_90 / KITCHEN_SCENE5_put_the_black_bowl_on_top_of_the_cabinet
Task language: "put the black bowl on top of the cabinet"
Task type: pick-and-place (wide metallic-rendered bowl -> +Y cabinet top surface, z=0.215)

Adapted from KITCHEN_SCENE4_put_the_black_bowl_on_top_of_the_cabinet (30/30) which
uses the same scene archetype: wide ~10.5cm metallic-rendered bowl at table level,
white-topped wooden cabinet on +Y side at z~0.21.

Seed-51 probe (KS5):
- Bowl center [0.690, -0.031, 0.025], ext [0.105, 0.110, 0.062], top_z 0.087.
  SAM3: "small bowl" 0.92, "metal bowl" 0.90, "bowl" 0.85, "black bowl" 0.33.
- Cabinet top center [0.656, 0.320, 0.214], ext [0.255, 0.190, 0.004], surface_z 0.215.
  SAM3: "cabinet top" 0.83, "white top" 0.84.

Strategy:
- Bowl is wider than gripper (10.5cm vs 8cm). Forced rim grasps at 0.95 x OBB radius
  along +/-X, +/-Y; descend deep below rim (top_z - 0.030); apply rim-offset
  compensation at placement.
- Lift to z=0.40 above cabinet top (surface_z + 0.20) before lateral transit.
- Release at surface_z + bowl_height + 0.005 to land bowl on cabinet top.
"""
import numpy as np
...
def select_bowl(rgb, depth_img, K, E):
    """Find the metallic/silver-rendered black bowl on the table.
    KS5 bowl renders silver -> "small bowl"/"metal bowl" score >0.9; "black bowl" only ~0.33.
    """
    candidates = []
    for prompt in ("small bowl", "metal bowl", "silver bowl", "bowl", "black bowl"):
        ...
        for m in masks[:6]:
            ...
            if e[2] > 0.12 or e[2] < 0.02:
                continue
            # Bowl on counter side (Y < 0.10) — not on cabinet top
            if c[1] > 0.10:
                continue
...
def select_cabinet_top(rgb, depth_img, K, E):
    """Find the on-table wooden cabinet top surface (z=0.21, +Y side)."""
    candidates = []
    for prompt in ("cabinet top", "white top", "top of cabinet", "wooden cabinet top"):
        masks = segment_sam3_text_prompt(rgb, prompt)
        for m in masks[:8]:
            mask = m["mask"].astype(np.uint8)
            pts = mask_to_world_points(mask, depth_img, K, E)
...
def attempt_grasp_at(grasp_xy, top_z, label, yaw_deg=0.0):
    ...
    straddle the rim wall from inside/outside. Aim for top_z - 0.030 like KS4.
    """
    open_gripper()
...
# ---------------- Grasp ----------------
# Bowl ~10.5cm > gripper 8cm. Use FORCED RIM POSITIONS at 0.95 * OBB radius.
...
for i, (gx, gy, label) in enumerate(attempts):
    ...
    if i < len(attempts) - 1:
        try:
            ...
            r2 = obs2["agentview"]
            d2_arr = r2["images"]["depth"]
            d2_img = d2_arr[:, :, 0] if d2_arr.ndim == 3 else d2_arr
            bc2, bp2, _ = select_bowl(r2["images"]["rgb"], d2_img, r2["intrinsics"], r2["pose_mat"])
            if bc2 is not None:
