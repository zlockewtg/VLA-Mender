...
Place: wicker basket (rim z ≈ 0.16; ~17–20cm wide; center near (0.51, 0.26, 0.11)).

Disambiguation:
  Scene contains TWO short cans of similar shape:
    * tomato sauce can (red label, R/B ≈ 1.46) — TARGET
    * other (alfredo/blue/yellow) can            (R/B ≈ 0.98) — distractor
  SAM3 'tomato sauce can' returns BOTH with comparable scores (~0.92). Use R/B
  ratio over the masked pixels to pick the redder one (validated chunk 3,
  tomato_tray, 30/30).

  Other scene objects: butter (flat, R/B=0.73), cream cheese (flat, R/B=0.74),
  basket (large XY). Filter cans by ext_z in [0.04, 0.10] to exclude flat boxes
  and tall bottles, and pos_z<0.10 to exclude wall fixtures.

Basket placement: reuses cream_basket pattern (chunk 3, 30/30).
  - Localize wicker basket; rim_z = p95 of basket pts z.
  - Transit at max(lift_z, rim_z + 0.20) to clear walls.
  - Release at rim_z + can_height + 0.05 (drop in free air).
...
def red_can_pick(rgb, candidates, threshold=1.2):
    ...
    for c in candidates:
        ...
        if rb > best_rb:
            best_rb = rb
            best = c
    if best is not None and best_rb < threshold:
        # Still return it; downstream may warn
        pass
    return best
...
# SAM3 returns both cans with similar scores; use R/B ratio to pick redder one.
...
can_cands = find_object(rgb, depth_img, K, E, can_prompts,
                        ...
                        pos_z_max=0.10, min_score=0.30, return_all=True)
can_cands = dedupe_candidates(can_cands, radius=0.05)
print(f"[CAN] {len(can_cands)} can-shaped candidates after dedupe")
for c in can_cands:
    ys, xs = np.where(c["mask"] > 0)
    if len(ys) >= 20:
...
# Release above rim by enough to drop the can in: rim + can_height + margin
