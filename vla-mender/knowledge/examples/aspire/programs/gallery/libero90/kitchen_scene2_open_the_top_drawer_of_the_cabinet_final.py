"""
Task code for: libero_90 / KITCHEN_SCENE2_open_the_top_drawer_of_the_cabinet
Task language: "open the top drawer of the cabinet"

Strategy (drawer-only, identical to KS1 — same cabinet geometry):
1. Localize top drawer handle via SAM3 prompts → filter to cabinet workspace
   x∈[0.40,0.85], y∈[-0.30,-0.10], z∈[0.04,0.30]; pick z∈[0.14,0.22] (top drawer).
2. Approach handle from +Y with horizontal gripper (quat_h points -y).
3. Push slightly past handle, close gripper, pull in +Y direction (28cm total).
4. Two-step escape and goto_home.

Validated reference: KS1 task_code.py.
KS2 confirmed handle (seed 51): c≈(0.672,-0.195,0.178) — identical to KS1.
SCENE NOTE: KITCHEN_SCENE2 has 3 black bowls on the table; ignored — we only need cabinet handle.
"""
import numpy as np
...
# Filter to top drawer: z in [0.14, 0.22] (top drawer handle ≈ 0.178)
