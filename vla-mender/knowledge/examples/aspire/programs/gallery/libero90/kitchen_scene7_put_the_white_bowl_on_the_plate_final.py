...
Task: pick a white bowl (sitting on top of an appliance/oven at z~0.22)
and place it ON the plate (on the table at z~0).

Spatial convention (KITCHEN scene, agentview camera):
  Camera at (~1.32, 0, 0.70) looking along -x.
  World +y = image-right.

Geometry (verified, see KITCHEN_SCENE7_put_the_white_bowl_to_the_right_of_the_plate v3):
...
SAM3 prompts (validated):
  Bowl: "small bowl" (0.90), "white bowl" (0.67), "bowl" (0.67).
  Plate: "round plate" (0.94), "plate" (0.86), "dinner plate" (0.83).

Strategy (adapted from sibling task put_the_white_bowl_to_the_right_of_the_plate, 29/30 = 96.7%):
  1. Settle physics with gripper toggles.
  2. Localize plate (high-confidence first via "round plate"), filter geometric.
  3. Localize bowl (filter shape so we don't pick the plate; require pos_z>0.10 to exclude
     potential table bowl confusion with elevated bowl).
  4. Compute target_xy = plate_center_xy (place ON plate, no lateral offset).
  5. Top-down rim-percentile XY grasp at bowl_top_z - 0.010, yaw=0.
  6. Lift to max(grasp_z + 0.20, 0.40) to clear appliance top.
  7. Lateral move via step_to(n=4) to above plate at lift z.
  8. Lower to release_z = plate_top_z + bowl_height - 0.010 + 0.003 (bowl bottom ≈ plate top + 3mm).
  9. Open gripper, retreat +0.20.

Use step_to() for all moves >5cm (goto_pose budget unreliable).
"""
...
# Elevated bowl (on appliance) — pos_z_min=0.10 to filter out plate-similar low candidates.
...
bowl = find_object(rgb, depth_img, K, E, bowl_prompts,
                   ...
                   pos_z_min=0.10, pos_z_max=0.40, min_score=0.30)
if bowl is None:
    # Relax pos_z_min if not found (some seeds may have bowl lower than expected)
    bowl = find_object(rgb, depth_img, K, E, bowl_prompts,
                       ext_xy_min=0.05, ext_xy_max=0.12, ext_z_min=0.02, ext_z_max=0.07,
...
# === Step 5: Compute target_xy (ON the plate, centered) ===
target_x = float(plate_center[0])
target_y = float(plate_center[1])
# Plate top-surface z (top 15% of plate pts → top of plate disc)
plate_top_z = float(np.percentile(plate_pts[:, 2], 85))
# Plate base z (underside ≈ table top)
table_z = float(np.percentile(plate_pts[:, 2], 5))
print(f"[TARGET] xy=({target_x:.3f}, {target_y:.3f}) plate_top_z={plate_top_z:.3f} table_z={table_z:.3f}")

# === Step 6: Pick bowl (rim-percentile XY for unbiased center) ===
...
close_gripper()
# Re-tighten — maintains grip on slipping rim
close_gripper()
...
# === Step 7: Lift ===
# Bowl was at z=0.22 (on appliance). Lift well above appliance to clear it.
# Incremental lift in small Z steps to prevent grip slip during arm reconfiguration.
lift_z = max(grasp_z + 0.20, 0.40)
for step_z in [grasp_z + 0.04, grasp_z + 0.10, grasp_z + 0.18, lift_z]:
    sp = np.array([bowl_xy[0], bowl_xy[1], step_z])
    j = solve_ik(sp.tolist(), quat.tolist())
    if j is not None:
        move_to_joints(j)
# Re-tighten after lift to fight any settling
close_gripper()
obs2 = get_observation()
print(f"[LIFT] pos={obs2['robot_cartesian_pos'][:3]} width={obs2['robot_cartesian_pos'][7]:.3f}")

# === Step 8: Move laterally above plate center ===
...
# We want bowl bottom = plate_top_z + small clearance so it lands ON the plate.
release_offset = bowl_height - 0.010  # tips above bowl bottom at grasp
release_z = plate_top_z + release_offset + 0.003  # bowl bottom 3mm above plate top
release_pos = np.array([target_x, target_y, release_z])
print(f"[RELEASE] z={release_z:.3f} (plate_top_z={plate_top_z:.3f}, bowl_h={bowl_height:.3f}, offset={release_offset:.3f})")
