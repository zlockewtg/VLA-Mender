...
- After ascending X sort: bowls[0]=back (min X), bowls[1]=middle (mid X), bowls[-1]=front (max X).
- Pick the MIDDLE bowl (bowls[1]), place on the BACK bowl (bowls[0]).
- GraspNet grasp on middle bowl rim, lift, transit through home_joint_position to avoid IK singularities,
  then approach above back bowl, descend & release with XY correction for grasp offset.
"""
import numpy as np
...
def run():
    ...
    #   bowls[0] = back  (smallest X)
    #   bowls[1] = middle
    #   bowls[-1]= front (largest X)
    # This task: pick MIDDLE, place on BACK.
    if len(bowls) >= 3:
        pick_bowl = bowls[1]      # middle
        place_bowl = bowls[0]     # back
    else:
        # Fallback: only 2 bowls — pick the one with larger X, place on the smaller X
        pick_bowl = bowls[-1]
        place_bowl = bowls[0]

    pick_ctr, pick_pts, pick_mask, _ = pick_bowl
    place_ctr, place_pts, place_mask, _ = place_bowl
    pick_top_z = float(np.percentile(pick_pts[:, 2], 90))
    place_top_z = float(np.percentile(place_pts[:, 2], 90))
    print(f"MIDDLE (pick): ({pick_ctr[0]:.3f},{pick_ctr[1]:.3f},{pick_ctr[2]:.3f}) top_z={pick_top_z:.3f}", flush=True)
    print(f"BACK   (place): ({place_ctr[0]:.3f},{place_ctr[1]:.3f},{place_ctr[2]:.3f}) top_z={place_top_z:.3f}", flush=True)

    open_gripper()
    quat = TOP_DOWN_QUAT.copy()

    # ---- Pre-grasp ----
    safe_move([pick_ctr[0], pick_ctr[1], 0.30], quat)

    # ---- GraspNet ----
    grasp_pos = None
    try:
        gposes, gscores = plan_grasp(obs["agentview"]["images"]["depth"], K, pick_mask)
        if gposes is not None and len(gposes) > 0:
            ...
            gp = best_world[:3, 3]
            if np.linalg.norm(gp[:2] - pick_ctr[:2]) < 0.08:
                grasp_pos = gp.copy()
    ...
    if grasp_pos is None:
        grasp_z = max(pick_top_z - 0.005, 0.005)
        grasp_pos = np.array([pick_ctr[0], pick_ctr[1], grasp_z])
        print(f"Centroid fallback: {grasp_pos}", flush=True)

    grasp_offset = grasp_pos[:2] - pick_ctr[:2]
    ...
    # ---- Approach above BACK bowl with XY offset correction (use refined held_offset) ----
    target_x = place_ctr[0] + held_offset[0]
    target_y = place_ctr[1] + held_offset[1]
    above = np.array([target_x, target_y, 0.30])
    print(f"Move above back (corrected): {above}", flush=True)
    safe_move(above, quat)
    obs_a = get_observation()
    ee_a = get_ee_pos(obs_a)
    print(f"  EE after approach={ee_a}", flush=True)

    # If IK overshot Z, retry without the X correction (just direct above back bowl)
    ...
    try:
        ...
        for m in sorted(masks_held2, key=lambda d: d["score"], reverse=True)[:6]:
            ...
            if d_to_arm < 0.15:
                ...
                # We want bowl at place_ctr — so move arm by (place_ctr - held_ctr2)
                delta_xy = place_ctr[:2] - held_ctr2[:2]
    ...
    # ---- Multi-step descent ----
    # IK clamps Z to ~0.23 due to kinematic floor; ask for low Z to push it down.
