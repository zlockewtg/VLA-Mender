...
- Sorting bowls by world-X gives front (X~0.48), middle (X~0.59), back (X~0.74).
- Plate: 'plate with red rims' (score>0.97). Median XY ≈ (0.64–0.68, 0.0±0.02).
  Surface z (85th-pctile) ≈ 0.004.
- "the bowl at the back" = bowl with largest X (~0.74-0.76, Y~0.13-0.18).

Strategy:
1. SAM3 'small bowl' returns 3 candidates with score≥0.93. Filter by reasonable
   X/Y/Z bounds, sort by X ascending. Pick LAST = back bowl.
...
def localize_bowls(rgb, depth_img, K, E):
    """Return list of (center, pts, mask, score) sorted by world-X ascending (front->back)."""
    ...
    bowls.sort(key=lambda b: b[0][0])  # front (small X) -> back (large X)
    return bowls
...
# 1. Find all bowls and pick "back" (largest X)
...
# Use the FIRST (smallest X = visual top = back) bowl
# In LIBERO KITCHEN_SCENE2, "back" means small X (away from camera = visual top of image).
target_center, target_pts, target_mask, target_score = bowls[0]
...
print(f"BACK bowl: ({bx:.3f},{by:.3f},{bz:.3f}) Z=[{b_zmin:.3f},{b_zmax:.3f}]", flush=True)
