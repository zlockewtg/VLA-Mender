"""
v6: Same as v5 but with mid-transport close_gripper() to maintain grip on flat butter.
    Also adds gripper close at intermediate transport waypoints.

Plus: re-grasp if dropped during transit (detect via gw read).
"""
import numpy as np
...
def localize_butters_all(rgb, depth, K, E):
    if depth.ndim == 3:
        depth = depth[:, :, 0]
...
butter = butters[0]
print(f"FRONT butter (target): xy=[{butter['cx']:.3f},{butter['cy']:.3f}]", flush=True)

drawer = localize_drawer_interior(rgb, depth_img, K, E)
if drawer is None:
    drawer_x = DRAWER_X_FALLBACK
    drawer_y_front = DRAWER_Y_FRONT_FALLBACK
    drawer_floor_z = DRAWER_FLOOR_Z_FALLBACK
else:
    drawer_x = (drawer['x_range'][0] + drawer['x_range'][1]) / 2.0
    y_min, y_max = drawer['y_range']
    drawer_y_front = y_max - 0.25 * (y_max - y_min)
    drawer_floor_z = drawer['floor_z']
print(f"PLACE: x={drawer_x:.3f} y={drawer_y_front:.3f} floor={drawer_floor_z:.3f}", flush=True)
...
# Step lateral with periodic close_gripper to combat drift
...
# WP3: above drawer at TRANSIT_Z
mid_xy = ((butter_x + drawer_x) / 2.0, (butter_y + drawer_y_front) / 2.0)

print(f"Transport waypoints:", flush=True)
for tag, wx, wy in [("mid", mid_xy[0], mid_xy[1]), ("above_drawer", drawer_x, drawer_y_front)]:
...
for tz in descent_tcp:
    j = solve_ik([drawer_x, drawer_y_front, tz], chosen_quat.tolist())
    if j is not None:
        try: move_to_joints(j)
        except Exception: break

obs_pp = get_observation()
print(f"At place: wrist=[{obs_pp['robot_cartesian_pos'][0]:.3f},{obs_pp['robot_cartesian_pos'][1]:.3f},{obs_pp['robot_cartesian_pos'][2]:.3f}] gw={obs_pp['robot_cartesian_pos'][-1]:.3f}", flush=True)
...
# Lift away
try:
    j = solve_ik([drawer_x, drawer_y_front, TRANSIT_Z], TOP_DOWN_QUAT.tolist())
    if j is not None: move_to_joints(j)
except Exception: pass

try:
    j = solve_ik([drawer_x, drawer_y_front + 0.20, TRANSIT_Z], TOP_DOWN_QUAT.tolist())
    if j is not None: move_to_joints(j)
except Exception: pass
